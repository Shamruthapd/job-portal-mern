...
router.put("/:id",auth,recruiterOnly,async (req,res)=>{
    try{
      ...
      if(!title || !company || !location || !description || !skillsRequired || skillsRequired.length===0 || !jobType || !experienceLevel || !lastDate)
        return res.status(400).json({ msg: "All fields are required" });
      const updatedJob= await Job.findByIdAndUpdate(req.params.id,{title,company,location,description,skillsRequired,jobType,experienceLevel,lastDate},{ new: true, runValidators: true });
